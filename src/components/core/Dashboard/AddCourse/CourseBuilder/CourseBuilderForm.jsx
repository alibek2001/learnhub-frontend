...
import { createSection, updateSection } from "../../../../../services/operations/mockCourseAPI"
...
export default function CourseBuilderForm() {
  ...
  const goToNext = () => {
    if (course.courseContent.length === 0) {
      toast.error("Пожалуйста, добавьте хотя бы один раздел")
      return;
    }
    if (course.courseContent.some((section) => section.subSection.length === 0)) {
      toast.error("Пожалуйста, добавьте хотя бы одну лекцию в каждый раздел")
      return;
    }
  ...
  return (
    <div className="space-y-8 rounded-2xl border-[1px] border-richblack-700 bg-richblack-800 p-6">
      <p className="text-2xl font-semibold text-richblack-5">Конструктор курса</p>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {/* Section Name */}
        <div className="flex flex-col space-y-2">
          <label className="text-sm text-richblack-5" htmlFor="sectionName">
            Название раздела <sup className="text-pink-200">*</sup>
          </label>
          <input
            id="sectionName"
            disabled={loading}
            placeholder="Добавьте раздел для создания курса"
        ...
        <div className="flex items-end gap-x-4">
          <IconBtn
            type="submit"
            disabled={loading}
            text={editSectionName ? "Изменить название раздела" : "Создать раздел"}
            outline={true}
          >
      ...
      <div className="flex justify-end gap-x-3">
        ...
        {/* Next button */}
        <IconBtn disabled={loading} text="Далее" onclick={goToNext}>
          <MdNavigateNext />
        </IconBtn>
      </div>
    </div>
